...
class LSHSimilarity:
    ...
    def _create_minhash(self, features):
        ...
        for i, value in enumerate(features):
            m.update(f"{i}:{int(value * 100)}".encode('utf-8'))
        return m
    
    def add_song(self, song_name, features):
        """Add a song to the LSH index"""
        minhash = self._create_minhash(features)
        self.minhashes[song_name] = minhash
        self.lsh.insert(song_name, minhash)
    
    def find_similar_songs(self, features):
